int Fun4All_G4_EICDetector(
                           const int nEvents = 1,
                           const char * inputFile = "./pythia.ep.20x250.RadCor=0.Q2gt1.50kevts(1).root",
                           ...
                           )
{
  ...
  bool do_svtx_eval = do_svtx_track && eval;
  ...
  bool do_cemc_eval = do_cemc_cluster && eval;

  bool do_hcalin = false;
  bool do_hcalin_cell = do_hcalin && true;
  bool do_hcalin_twr = do_hcalin_cell && true;
  bool do_hcalin_cluster = do_hcalin_twr && true;
  bool do_hcalin_eval = do_hcalin_cluster && eval;
  ...
  bool do_hcalout_eval = do_hcalout_cluster && eval;
  ...
  bool do_EEMC_eval = do_EEMC_cluster && eval;
  ...
  bool do_jet_eval = do_jet_reco && eval;

  bool do_fwd_jet_reco = false;
  bool do_fwd_jet_eval = do_fwd_jet_reco && eval;
  ...
  if(upsilons)
    {
      ...
      // Which vector meson is produced?
      const enum particle_type{ Jpsi , Upsilon };
      const enum particle_state{ ground , first , second }; // first excited, second excited, for Upsilon
      particle_type = Jpsi;
      particle_state = ground;

      if(particle_type==Jpsi)
	{
	  switch(particle_state){
	  default : // J/Psi default
	    vgen->set_mass(3.096); //GeV/c^2
	    vgen->set_width(92.9e-6);
	    break;
	  }
      ...
      cout << "J/Psi generator for istate = " << istate << " created and registered "  << endl;
	}
      else if(particle_type==Upsilon)
	{
	  switch(particle_state){
	  case ground : // Upsilon(1S)
	    vgen->set_mass(9.46);
	    vgen->set_width(54.02e-6);
	    break;
	  case first  : // Upsilon(2S)
	    vgen->set_mass(10.0233);
	    vgen->set_width(31.98e-6);
	    break;
	  case second : // Upsilon(3S)
